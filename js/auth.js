...
async function handleLogin(event) {
    ...
    try {
        ...
        if (error.code === 'auth/user-not-found') msg = 'ไม่พบอีเมลนี้ในระบบ (กรุณาสมัครสมาชิกก่อน)';
        if (error.code === 'auth/wrong-password') msg = 'รหัสผ่านไม่ถูกต้อง';
        if (error.code === 'auth/invalid-email') msg = 'รูปแบบอีเมลไม่ถูกต้อง';
        if (error.code === 'auth/operation-not-allowed') msg = 'ยังไม่ได้เปิดใช้งาน Email/Password ใน Firebase Console (ดูคู่มือ)';

        console.error('Login Error:', error); // Log for debugging

        // Append error code if it's an unhandled error
        if (!msg.includes('(') && error.code) {
            msg += ` (${error.code})`;
        } else if (!msg.includes('(') && error.message) {
            msg += ` (${error.message})`;
        }
